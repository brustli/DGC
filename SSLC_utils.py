...
class SSLCL(nn.Module):
    ...
    def __init__(self, pos_focus_param, neg_focus_param, sample_weight_param, pos_aug_param, label_loss_param,
                 num_classes, device):
        super().__init__()
    ...
    def soft_HGR_correlation(self, X_embs, Y_embs):
        ...
        X_embs_corr = torch.cov(zero_mean_X_embs)  # (bs, bs)
    ...
    def forward(self, f_t_embs, label_embs, labels, f_ta_embs = None, f_tv_embs = None, f_tav_embs = None):

        #拉直feature
        f_t_embs = f_t_embs.reshape(-1, 300)
        ...
        # todo 这里逻辑是在labelemb的时候映射一个字典出来而不是真的按数据集映射
        batch_label_embs = torch.stack(([label_embs[label] for label in labels]))

        all_labels = self.label_indices.expand_as(torch.zeros(self.num_samples, self.num_classes)).to(
            self.device).transpose(1, 0).clone()
        all_labels[0] = labels
        ...
        if self.pos_aug_param == True:
            positive_pairs_loss_weight = torch.FloatTensor([1 / 3, 2 / 3, 2 / 3, 1.0]).reshape(-1, 1).expand_as(
                torch.zeros(4, feature_label_probs[:4].shape[-1])).to(self.device).clone()
            feature_label_pos_loss = torch.mean(
                self.positive_pairs_loss(feature_label_probs[:4]) * positive_pairs_loss_weight, dim=0)
        ...
        extended_labels = self.label_indices.expand_as(torch.zeros(self.num_classes, self.num_classes)).to(
            self.device).transpose(1, 0).clone()
...
class LabelEmbedding(nn.Module):
    ...
    def forward(self, labels):
        ...
        embs = self.relu(self.embedding_layer(labels))

        # print(embs)

        out = self.linear_layer(embs)
        out_embs = self.dropout(out)
